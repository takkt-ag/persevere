...
struct State {
    ...
    #[serde(with = "crate::de::completed_parts")]
...
pub(crate) enum Upload {
    /// Start the upload of a file to S3.
    ///
...
impl Upload {
    pub(crate) async fn run(self) -> Result<()> {
        match self {
            Upload::Start(start) => start.run().await,
            Upload::Resume(resume) => resume.run().await,
            Upload::Abort(abort) => abort.run().await,
        }
    }
}
...
async fn upload(s3: &aws_sdk_s3::Client, state_file: &Path, state: &mut State) -> Result<()> {
    ...
    for part_number in first_part_number..(MINIMUM_PART_NUMBER + state.number_of_parts) {
        ...
        if let Some(error) = last_retry_error {
            ...
            error!(
                "persevere upload resume --state-file '{}'",
                state_file.display()
            );
            return Err(error);
        }
